<template>
  <div class="helpers-page">
    <el-card>
      ...
      <el-table :data="helperList" border v-loading="loading">
...
import { getHelperList, createHelper, updateHelper, deleteHelper } from '@/api/helpers'
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`确认删除护工【${row.name}】吗？`, '提示', {
      type: 'warning'
    })
    await deleteHelper(row.id)
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        if (isEdit.value) {
          await updateHelper(currentId.value, submitData)
          ElMessage.success('更新成功')
        } else {
          await createHelper(submitData)
